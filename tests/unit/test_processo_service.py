...
class TestProcessoService:
    ...
    def test_get_processo_metricas(self, service, mock_repository, db_mock):
        ...
        pagina_stats_mock = Mock()
        pagina_stats_mock.total_paginas = 100
        pagina_stats_mock.total_bytes = 1024000
        
        # Mock das queries
        with patch.object(db_mock, 'query') as query_mock:
            # Primeira query (exec_stats)
            filter_mock = Mock()
            first_mock = Mock(return_value=exec_stats_mock)
            filter_mock.first = first_mock
            
            query_mock.return_value.filter.return_value = filter_mock
            
            # Para a segunda query (pagina_stats), precisamos configurar diferente
            def query_side_effect(*args):
                query_result = Mock()
                
                if hasattr(args[0], '__len__'):
                    # Segunda query (páginas)
                    join_mock = Mock()
                    filter_mock2 = Mock()
                    first_mock2 = Mock(return_value=pagina_stats_mock)
                    
                    filter_mock2.first = first_mock2
                    join_mock.filter = Mock(return_value=filter_mock2)
                    query_result.join = Mock(return_value=join_mock)
                else:
                    # Primeira query (execuções)
                    filter_mock3 = Mock()
                    first_mock3 = Mock(return_value=exec_stats_mock)
                    filter_mock3.first = first_mock3
                    query_result.filter = Mock(return_value=filter_mock3)
